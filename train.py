...
def train_one_epoch(model, optimizer, data_loader, lr_scheduler, epoch, print_freq, loss_scaler, clip_grad):
    ...
    for i, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # first forward pass
        with torch.cuda.amp.autocast():
            loss_dict_main, attention_map = model(image, sentences, l_mask=attentions, targets=targets, attention_map=None, is_proxy=False)
            
        total_loss_main = loss_dict_main['total_loss'] * 0.5
        # Check if the loss is NaN or Inf
        if torch.isnan(total_loss_main) or torch.isinf(total_loss_main):
            print(f"🚫 [Epoch {epoch} | Iter {i}] [NaN/Inf detected] Skipping this batch (main loss).")
            skipped_batches += 1
            optimizer.zero_grad()
            continue
        loss_scaler(loss_dict_main['total_loss'] * 0.5, optimizer, clip_grad=clip_grad, parameters=model.parameters(), update_grad=False)
        
        # second forward pass
        with torch.cuda.amp.autocast():
            loss_dict_proxy = model(image, sentences, l_mask=attentions, targets=targets,attention_map=attention_map, is_proxy=True)
        total_loss_proxy = loss_dict_proxy['total_loss'] * 0.5
        # Check if the loss is NaN or Inf
        if torch.isnan(total_loss_proxy) or torch.isinf(total_loss_proxy):
            print(f"🚫 [Epoch {epoch} | Iter {i}] [NaN/Inf detected] Skipping this batch (proxy loss).")
            skipped_batches += 1
            optimizer.zero_grad()
            continue

        grad_norm = loss_scaler(loss_dict_proxy['total_loss']* 0.5, optimizer, clip_grad=clip_grad, parameters=model.parameters(), update_grad=True)
       
       
        # Check if the grad_norm is NaN or Inf
        if torch.isnan(grad_norm) or torch.isinf(grad_norm):
            print(f"🚨 [Epoch {epoch} | Iter {i}] [NaN/Inf detected] Skipping optimizer step.")
            skipped_batches += 1
            optimizer.zero_grad()
            continue
        ...
        metric_logger.update(org_loss=loss_dict_main['total_loss'].item())
        metric_logger.update(sub_loss=loss_dict_proxy['total_loss'].item())
    if skipped_batches > 0:
        print(f"⚠️ [Epoch {epoch}] Skipped {skipped_batches} batches due to NaN/Inf.")
...
def main(args, distributed):
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        if epoch % 2 == 0 or epoch >= args.epochs - 16 or epoch == 1:
